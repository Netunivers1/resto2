...
use AppBundle\Entity\Platdujour;
...
class PlatdujourController extends Controller {
     ...
     */
    public function getplatdujourAction(Request $request, $paginate = 1, $page = 1, $limit = 5) {
        $Menu = $this->get('doctrine.orm.entity_manager')
                ->getRepository('AppBundle:Platdujour')
    ...
    public function removeplatdujourAction(Request $request, $id) {
        ...
        $Vins2 = $em->getRepository('AppBundle:Platdujour')
                    ->find($request->get('id'));
    ...
    public function updateplatdujourAction(Request $request) {
        $entity_manager = $this->getDoctrine()->getManager();
        $product = $entity_manager->getRepository(Platdujour::class)->find($request->get('id'));
